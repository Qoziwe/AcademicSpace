...
export interface FlashcardDeckSeed {
  id: string;
  title: string;
  source: 'text' | 'image';
  createdAt: string;
  /** Не меняется после создания — знаменатель прогресса на FLASHCARDS_STUDY. */
  cardsTotal: number;
  /** Карточки, которые ещё не свайпнули влево ("запомнил"). */
  cards: FlashcardSeed[];
}

/** Лимит одновременно хранимых колод. Free/Premium; удаление освобождает слот. */
export const FLASHCARD_QUOTAS = { free: 1, premium: 10 } as const;

/** XP за полностью пройденную колоду (все карточки "запомнены"). */
export const FLASHCARD_DECK_XP = 30;

/**
 * "Нейронка" на моках не читает реальный текст/фото — как и
 * `ANALYSIS_PREVIEW_BLOCKS`, отдаёт один из заготовленных наборов (ротация
 * по числу уже созданных колод, см. `mocks/store.ts.createFlashcardDeck`).
 */
export const FLASHCARD_MOCK_BANK: { title: string; cards: FlashcardSeed[] }[] = [
  {
    title: 'География: Тихий океан',
    cards: [
      {
        id: 'q1',
        question: 'Сколько материков омывает Тихий океан?',
        answer:
          'Все шесть обитаемых материков — Евразию, Африку, Северную и Южную Америку, Австралию и Антарктиду.',
      },
      {
        id: 'q2',
        question: 'Самая глубокая точка Тихого океана?',
        answer: 'Марианская впадина, около 11 034 м — глубочайшая точка Мирового океана.',
      },
      {
        id: 'q3',
        question: 'Какой это океан по площади среди остальных?',
        answer: 'Самый большой: занимает почти треть всей поверхности Земли.',
      },
      {
        id: 'q4',
        question: 'Что такое Тихоокеанское огненное кольцо?',
        answer:
          'Цепь вулканов и зон землетрясений по границам Тихого океана — около 75% активных вулканов планеты.',
      },
    ],
  },
  {
    ...
    cards: [
      { id: 'q1', question: 'Формула дискриминанта?', answer: 'D = b² − 4ac.' },
      {
        id: 'q2',
        question: 'Когда уравнение не имеет действительных корней?',
        answer: 'Когда D < 0.',
      },
      {
        id: 'q3',
        question: 'Формула корней через дискриминант?',
        answer: 'x = (−b ± √D) / (2a).',
      },
      {
        id: 'q4',
        question: 'Теорема Виета для x² + px + q = 0?',
        answer: 'x₁ + x₂ = −p, x₁ · x₂ = q.',
      },
    ],
  },
  {
    ...
    cards: [
      {
        id: 'q1',
        question: 'Первый закон Ньютона своими словами?',
        answer:
      ...
      },
      {
        ...
        answer: 'F = ma — сила равна произведению массы на ускорение.',
      },
      {
        id: 'q3',
        question: 'Третий закон Ньютона?',
        answer: 'Сила действия равна силе противодействия и направлена в противоположную сторону.',
      },
      {
        id: 'q4',
