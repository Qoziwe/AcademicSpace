...
> Фаза 6 (QA-проход).
>
> **Фаза 6 завершена:** QA-проход по всем 30 экранам (headless Chrome,
> обе темы × Free/Premium × анкета заполнена/нет) — 0 неготовых страниц,
> весь интерактив живой (отправка сообщения, чек-боксы модуля, свитч темы,
> выбор фильтров). Все 35 route-файлов сверены с реестром: `auth`/`premium`
> совпадают. Guard'ы: Free по deep link на любой из 7 Premium-only роутов →
> `/subscription/offer`; без сессии на любой `auth:true` роут → `/welcome`.
> §6-гейтинг подтверждён (дашборд: замки на плитках + скрытая вкладка
...
>
> **Фаза 9 — Умные карточки:** новый функционал сверх исходных 30 экранов
> дизайн-референса (`docs/00-roadmap.md`). 4 роута
> (`FLASHCARDS_LIST/CREATE/GENERATING/STUDY`), вход — плитка «Карточки» на
> дашборде. Free — 1 колода одновременно, Premium — 10, удаление
> освобождает слот. Свайп влево — «запомнил» (карточка уходит навсегда,
> закрытая колода начисляет XP в журнал), свайп вправо — перевернуть
> лицо/оборот, ещё раз вправо — следующая карточка. Мок-слой:
> `services/api/flashcards.ts`, `mocks/handlers/flashcards.ts`, контракт —
> `docs/api-contract.md` §Flashcards.
...
| 36 | FLASHCARDS_LIST | `/flashcards` | Умные карточки | нет в дизайне | false | Фаза 9. Список колод; квота на CTA «Создать» (Free 1 / Premium 10) | ✅ |
| 37 | FLASHCARDS_CREATE | `/flashcards/create` | Новая колода | нет в дизайне | false | Фаза 9. Текст/фото (мок file-picker, как PORTFOLIO_UPLOAD) | ✅ |
| 38 | FLASHCARDS_GENERATING | `/flashcards/generating` | Генерация карточек | нет в дизайне | false | Фаза 9. Навy, как ANALYSIS_LOADING; переход ведёт вызывающий экран | ✅ |
| 39 | FLASHCARDS_STUDY | `/flashcards/:deckId` | Колода карточек | нет в дизайне | false | Фаза 9. `<FlashcardStack>`: свайп влево — запомнил, вправо — перевернуть/дальше | ✅ |

Итого: **34 реальных экрана** (30 из Фаз 0–7 — 24 из исходного счётчика
прототипа + `PLAN_SELECTION`, которого физически нет в файле, но который
требует реестр роутов — + 4 из Фазы 9 «Умные карточки», сверх дизайн-
референса) + 2 глобальных компонента (Tab Bar, Offline banner).
