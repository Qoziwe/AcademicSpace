...
## Фаза 8 — Бекенд (Flask) — реализована (8.0–8.8)
...
Осознанно не реализовано (см. `docs/api-contract.md` §Адаптер-seam и
§TBD): Focus tools (сохранение сессий/трекеров), Settings/notifications
preferences, создание задачи из карточки-предложения в чате
(`POST /ai/chat/modules` — не описан в контракте, вне плана), реальный
платёжный провайдер, продакшн-деплой бекенда, облачное хранилище.
